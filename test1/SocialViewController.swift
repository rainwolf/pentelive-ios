...
class SocialViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, GADBannerViewDelegate, GADInterstitialDelegate {
    ...
    init(player: PentePlayer) {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.add, target: self, action: #selector(addFollowing))
    }
...
//                print(htmlString)
    ...
    func loadFollowersing() {
        do {
            followers.removeAll()
            following.removeAll()
            let followersing = try String(contentsOf: URL(string: "https://www.pente.org/gameServer/mobile/followers.jsp")!, encoding: String.Encoding.utf8)
