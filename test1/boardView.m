...
- (id)initWithCoder:(NSCoder *)aCoder {
    if (self = [super initWithCoder:aCoder]) {
        ...
        blackStoneView =
            [[StoneView alloc] initWithFrame:CGRectMake(0, 0, h, h)];
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i < gridSize; ++i) {
        ...
        CGContextAddLineToPoint(context, self.bounds.size.width - margin,
                                margin + i * margin * 2);
        CGContextStrokePath(context);
        CGContextMoveToPoint(context, margin + i * margin * 2, margin);
        CGContextAddLineToPoint(context, margin + i * margin * 2,
    ...
    if (go) {
        ...
        int r = gridSize - 1 - l;
        circle = CGRectMake(margin + 2 * c * margin - margin / 4,
                            margin + 2 * c * margin - margin / 4, margin / 2,
                            margin / 2);
        CGContextAddEllipseInRect(context, circle);
        CGContextFillPath(context);
        circle = CGRectMake(margin + 2 * l * margin - margin / 4,
                            margin + 2 * l * margin - margin / 4, margin / 2,
                            margin / 2);
        CGContextAddEllipseInRect(context, circle);
        CGContextFillPath(context);
        circle = CGRectMake(margin + 2 * l * margin - margin / 4,
                            margin + 2 * r * margin - margin / 4, margin / 2,
                            margin / 2);
        CGContextAddEllipseInRect(context, circle);
        CGContextFillPath(context);
        circle = CGRectMake(margin + 2 * r * margin - margin / 4,
                            margin + 2 * r * margin - margin / 4, margin / 2,
                            margin / 2);
        CGContextAddEllipseInRect(context, circle);
        CGContextFillPath(context);
        ...
        if (gridSize > 9) {
            circle = CGRectMake(margin + 2 * l * margin - margin / 4,
                                margin + 2 * c * margin - margin / 4,
                                margin / 2, margin / 2);
            CGContextAddEllipseInRect(context, circle);
            CGContextFillPath(context);
            circle = CGRectMake(margin + 2 * c * margin - margin / 4,
                                margin + 2 * l * margin - margin / 4,
                                margin / 2, margin / 2);
            CGContextAddEllipseInRect(context, circle);
            CGContextFillPath(context);
            circle = CGRectMake(margin + 2 * c * margin - margin / 4,
                                margin + 2 * r * margin - margin / 4,
                                margin / 2, margin / 2);
            CGContextAddEllipseInRect(context, circle);
            CGContextFillPath(context);
            circle = CGRectMake(margin + 2 * r * margin - margin / 4,
        ...
        CGRect circle =
            CGRectMake(margin + 12 * margin - margin / 2,
                       margin + 12 * margin - margin / 2, margin, margin);
        CGContextAddEllipseInRect(context, circle);
        CGContextStrokePath(context);
        circle.origin.x =
            self.bounds.size.width - margin - 12 * margin - margin / 2;
        CGContextAddEllipseInRect(context, circle);
        CGContextStrokePath(context);
        circle.origin.x =
            self.bounds.size.width - margin - 12 * margin - margin / 2;
    ...
    for (int i = 0; i < gridSize; ++i) {
        for (int j = 0; j < gridSize; ++j) {
            if (abstractBoard[i][j] > 0) {
                circle = CGRectMake(j * 2 * margin, i * 2 * margin, 2 * margin,
                                    2 * margin);
                CGPoint centre =
                    CGPointMake(circle.origin.x + margin - margin / 6,
                                circle.origin.y + margin - margin / 6);
                ...
                CGFloat components[8] = {
                    start, start, start, 1.0,  // Start color
                ...
                CGContextAddEllipseInRect(context, circle);
                CGContextSetShadow(context, CGSizeMake(margin / 6, margin / 6),
                                   0);
                CGContextFillPath(context);
                CGContextAddEllipseInRect(context, circle);
                CGContextClip(context);
                CGContextDrawRadialGradient(context, myGradient, centre, 0.0f,
                                            centre, 5 * margin / 4, 0);
    ...
    for (NSNumber *stone in whiteDeadStones) {
        int stoneInt = stone.intValue, i = stoneInt / gridSize,
            j = stoneInt % gridSize;
        circle =
    ...
    for (NSNumber *stone in whiteTerritory) {
        int stoneInt = stone.intValue, i = stoneInt / gridSize,
            j = stoneInt % gridSize;
        circle = CGRectMake(j * 2 * margin + margin - width / 2,
                            i * 2 * margin + margin - width / 2, width, width);
        CGContextSaveGState(context);
        CGContextTranslateCTM(context, circle.origin.x, circle.origin.y);
    ...
    if (lastConnect6Move > -1) {
        ...
        circle = CGRectMake(j * 2 * margin + 2 * margin / 3,
                            i * 2 * margin + 2 * margin / 3, 2 * margin / 3,
                            2 * margin / 3);
        CGContextAddEllipseInRect(context, circle);
        CGContextFillPath(context);
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor blackColor].CGColor);
    CGFloat circleSide =
        fill ? self.bounds.size.width : self.bounds.size.width / 1.2;
    ...
    CGPoint centre = CGPointMake(circle.origin.x + margin - margin / 6,
                                 circle.origin.y + margin - margin / 6);
    size_t num_locations = 2;
    CGFloat locations[2] = {0.0, 1.0};
    CGFloat start = 150.0 / 255.0;
    ...
    CGContextAddEllipseInRect(context, circle);
    CGContextClip(context);
    CGContextDrawRadialGradient(context, myGradient, centre, 0.0f, centre,
                                5 * margin / 4, 0);
    ...
    //    //    circle = CGRectMake(0.25,
    //    0.25,self.bounds.size.width-0.5,self.bounds.size.height-0.5); circle =
