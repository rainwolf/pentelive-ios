...
//*)nibBundleOrNil
//{
//    self = [super initWithNibName:@"IASKAppSettingsViewController"
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if ([[NSUserDefaults standardUserDefaults]
            boolForKey:@"registrationSuccess"]) {
        //        NSLog([[[self.tableView cellForRowAtIndexPath: [NSIndexPath
        //        indexPathForRow:0 inSection:1]] textLabel] text]);
        //        NSLog(NSLocalizedString(@"Username:",nil));
        if ([[[[self.tableView
                cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0
                                                         inSection:1]]
                textLabel] text]
                isEqualToString:NSLocalizedString(@"Username:", nil)]) {
            [self.tableView
                cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0
                                                         inSection:1]]
        ...
        if ([[[[self.tableView
                cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1
                                                         inSection:1]]
                textLabel] text]
                isEqualToString:NSLocalizedString(@"Password:", nil)]) {
            [self.tableView
                cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1
                                                         inSection:1]]
                .userInteractionEnabled = NO;
    ...
    //    NSLog([NSString stringWithFormat:@"kitty viewDidAppear %@",[self
    //    navC]]);
}
...
- (void)viewWillAppear:(BOOL)animated {
    if ([[NSUserDefaults standardUserDefaults]
        ...
        if (self.navC.player && self.navC.player.subscriber) {
            self.hiddenKeys =
                [NSSet setWithObjects:@"RestorePurchaseButton",
                                      @"SubscribeButton", @"SignUpButton",
                                      @"SignUpLabel", @"personalizeAds", nil];
            //            self.hiddenKeys = [NSSet
            //            setWithObjects:@"SignUpButton",
            //            @"SignUpLabel", nil];
        } else {
            self.hiddenKeys =
                [NSSet setWithObjects:@"SignUpButton", @"manageSubscriptions",
                                      @"SignUpLabel", nil];
        }
    } else {
        self.hiddenKeys =
            [NSSet setWithObjects:@"RestorePurchaseButton", @"SubscribeButton",
                                  @"manageSubscriptions", nil];
    }
    ...
    //    NSLog([NSString stringWithFormat:@"kitty2
    //    %@",self.navigationController]); NSLog([NSString
    //    stringWithFormat:@"kitty4 %@",self.parentViewController]);
    [super viewDidLoad];
    //    [[NSUserDefaults standardUserDefaults] setBool:YES
    //    forKey:@"shouldSendReceipt"]; NSURL *receiptURL = [[NSBundle
    //    mainBundle] appStoreReceiptURL]; NSData *receipt = [NSData
    //    dataWithContentsOfURL:receiptURL];
    //
    //    NSString *url =
    //    @"https://www.pente.org/gameServer/iOSReceiptValidation"; NSString
    //    *postString = [NSString stringWithFormat:@"name=%@&receipt=%@",
    //    [[NSUserDefaults standardUserDefaults] stringForKey:usernameKey],
    //    [self URLEncodedString_ch:[receipt
    //    base64EncodedStringWithOptions:0]]];
    //
    //    NSData *postData = [postString dataUsingEncoding:NSASCIIStringEncoding
    //    allowLossyConversion:YES]; NSString *postLength = [NSString
    ...
    //        forKey:@"shouldSendReceipt"]; [TSMessage
    ...
    //                                              title: @"Purchase
    //                                              registration failed"
    //                                           subtitle: @"The app will retry
    //                                           purchase registration at
    //                                           pente.org every time the app
    //                                           starts"
    //                                              image:nil
    //                                               type:
...
- (void)settingsViewController:(IASKAppSettingsViewController *)sender
    ...
    if ([specifier.key isEqualToString:@"SignupButton"]) {
        ...
        NSString *passwordVerification =
            [defaults objectForKey:@"passwordVerification"];
        ...
        NSString *postString = [NSString
            stringWithFormat:
                @"name=%@&registerPassword=%@&registerPasswordConfirm="
                @"%@&registerEmail=%@&agreePolicy=Y",
                username, password, passwordVerification,
                [self URLEncodedString_ch:emailAddress]];
    ...
    //        initWithFile:@"MoreSettings" specifier: specifier]; [self.navC
    ...
    if ([specifier.key isEqualToString:@"inviteFriendsButton"]) {
        if ([MFMailComposeViewController canSendMail]) {
            ...
            //            arrayWithObjects:@"rainwolf@submanifold.be", nil];
            ...
            [mailer
                setMessageBody:
                    [NSString
                        stringWithFormat:
                            NSLocalizedString(
                                @"<p>You can play with me on your <a "
                                @"href=\"https://itunes.apple.com/us/app/"
                                @"pente-live/"
                                @"id595426592?ls=1&mt=8\">iPhone</a> or <a "
                                @"href=\"https://play.google.com/store/apps/"
                                @"details?id=be.submanifold.pentelive\">"
                                @"Android "
                                @"Phone</a> <br> My username is %@",
                                nil),
    ...
    if ([specifier.key isEqualToString:@"GameRulesButton"]) {
        PenteWebViewController *webViewController =
            [[PenteWebViewController alloc]
                initWithAddress:
                    @"https://www.pente.org/help/playGameRulesMobile.jsp"];
    ...
    if ([specifier.key isEqualToString:@"penteOrgPreferencesButton"]) {
        PenteWebViewController *webViewController =
            [[PenteWebViewController alloc]
                initWithAddress:
                    @"https://www.pente.org/gameServer/myprofile/prefs"];
        [self.navigationController pushViewController:webViewController
                                             animated:YES];
    }
    if ([specifier.key isEqualToString:@"penteOrgPrivacyPolicyButton"]) {
        PenteWebViewController *webViewController =
            [[PenteWebViewController alloc]
                initWithAddress:@"https://www.pente.org/help/"
                                @"helpWindow.jsp?file=privacyPolicy"];
        [self.navigationController pushViewController:webViewController
                                             animated:YES];
    }
    if ([specifier.key isEqualToString:@"penteOrgEULAButton"]) {
        PenteWebViewController *webViewController =
            [[PenteWebViewController alloc]
                initWithAddress:
                    @"https://www.pente.org/help/helpWindow.jsp?file=eula"];
        [self.navigationController pushViewController:webViewController
                                             animated:YES];
    }
    if ([specifier.key isEqualToString:@"ForgotPasswordButton"]) {
        PenteWebViewController *webViewController =
            [[PenteWebViewController alloc]
                initWithAddress:
                    @"https://www.pente.org/gameServer/forgotpassword.jsp"];
        [self.navigationController pushViewController:webViewController
                                             animated:YES];
    }
...
- (void)showSubscribeInfo {
    ((PenteNavigationViewController *)self.navigationController).showSubscribe =
        NO;
    ...
    NSString *subscribeString =
        NSLocalizedString(@"\u2022 remove play limits\n\u2022 request undo in "
                          @"turn-based games\n\u2022 change your name color or "
                          @"avatar\n\u2022 access the database!",
                          nil);
    ...
    //        UILabel *priceText = [[UILabel alloc] initWithFrame:CGRectMake(0,
    //        0, self.view.bounds.size.width*9/10, CGFLOAT_MAX)];
    UILabel *priceText = [[UILabel alloc]
        initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width * 9 / 10,
                                 50)];
    [priceText setLineBreakMode:NSLineBreakByWordWrapping];
    [priceText setNumberOfLines:0];
    NSString *priceString =
        [NSString stringWithFormat:NSLocalizedString(@"only %@ / year", nil),
                                   formattedPrice];
    ...
    UILabel *clearInfoText = [[UILabel alloc]
        initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width * 9 / 10,
                                 CGFLOAT_MAX)];
    [clearInfoText setLineBreakMode:NSLineBreakByWordWrapping];
    [clearInfoText setNumberOfLines:0];
    NSString *clearInfoString = NSLocalizedString(
        @"(This subscription auto-renews every year and can be canceled up to "
        @"24hrs before renewal. Payment will be charged on your iTunes "
        @"account. "
        @"Renewal will occur 24hrs before the end of the subscription period "
        @"at "
        @"the same cost. Subscriptions can be canceled in the settings with "
        @"\"Manage Subscription\".)",
        nil);
    ...
    UILabel *privacyPolicyAndTOSText = [[UILabel alloc]
        initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width * 9 / 10,
                                 CGFLOAT_MAX)];
    ...
    NSString *privacyPolicyAndTOSString =
        NSLocalizedString(@"View our Privacy Policy and TOS", nil);
    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc]
        initWithString:privacyPolicyAndTOSString];
    [attStr addAttribute:NSForegroundColorAttributeName
                   value:[UIColor blueColor]
                   range:NSMakeRange(0, privacyPolicyAndTOSString.length)];
    ...
    UITapGestureRecognizer *tapGestureRecognizer =
        [[UITapGestureRecognizer alloc]
            initWithTarget:self
                    action:@selector(openPrivacyPolicyAndTOS)];
    ...
    UILabel *eulaText = [[UILabel alloc]
        initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width * 9 / 10,
                                 CGFLOAT_MAX)];
    ...
    [attStr addAttribute:NSForegroundColorAttributeName
                   value:[UIColor blueColor]
                   range:NSMakeRange(0, eulaString.length)];
    [attStr addAttribute:NSUnderlineStyleAttributeName
                   value:@(NSUnderlineStyleSingle)
                   range:NSMakeRange(0, eulaString.length)];
    ...
    [subscribeButton setTitleColor:[UIColor colorWithRed:0
                                                   green:0.0
                                                    blue:0.9
                                                   alpha:1]
                          forState:UIControlStateNormal];
    [subscribeButton
        setTitle:NSLocalizedString(@"\u2606      subscribe      \u2606", nil)
        forState:UIControlStateNormal];
    [subscribeButton addTarget:self
                        action:@selector(subscribe:)
              forControlEvents:UIControlEventTouchUpInside];
    [subscribeButton
        setFrame:CGRectMake(0, 0, self.view.bounds.size.width * 8 / 9, 60)];
    //        [subscribeButton setImage:[UIImage imageNamed:@"subscribeIcon"]
...
- (void)imagePickerController:(UIImagePickerController *)pickr
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        if (image) {
            UIImage *newImg = [self imageWithImage:image
                                      scaledToSize:scaleSize];
            NSData *imageData = UIImageJPEGRepresentation(newImg, 0.75f);
            //            NSLog(@"kitty %f and size %lu", scaleSize(unsigned
            ...
            while ([imageData length] > 65535) {
                ...
                //                NSLog(@"kitty %f and size %lu",
                //                scaleSize(unsigned long), [imageData length]);
            }

            NSString *urlString =
                @"https://www.pente.org/gameServer/changeAvatar";
            if (development) {
...
- (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGFloat)newMaxSize {
    ...
    UIGraphicsBeginImageContextWithOptions(
        CGSizeMake(image.size.width * ratio, image.size.height * ratio), NO,
        0.0);
    [image drawInRect:CGRectMake(0, 0, image.size.width * ratio,
...
- (void)checkAndUpdateSettings {
    if (self.navC.player &&
        self.navC.player.emailMe !=
            [[NSUserDefaults standardUserDefaults] boolForKey:@"emailMe"]) {
        NSString *url =
            @"https://www.pente.org/gameServer/changeEmailPreference";
        if (development) {
            url = @"https://localhost/gameServer/changeEmailPreference";
        }
        [self.navC.player setEmailMe:[[NSUserDefaults standardUserDefaults]
                                         boolForKey:@"emailMe"]];
        NSString *postString = [NSString
            stringWithFormat:@"emailMe=%@",
                             (self.navC.player.emailMe ? @"Y" : @"N")];
    ...
    if (self.navC.player && self.navC.player.personalizeAds !=
                                [[NSUserDefaults standardUserDefaults]
                                    boolForKey:PERSONALIZEADSKEY]) {
        NSString *url = @"https://www.pente.org/gameServer/changeAdsPreference";
        if (development) {
            url = @"https://localhost/gameServer/changeAdsPreference";
        }
        [self.navC.player
            setPersonalizeAds:[[NSUserDefaults standardUserDefaults]
                                  boolForKey:PERSONALIZEADSKEY]];
        NSString *postString = [NSString
            stringWithFormat:@"personalizeAds=%@",
                             (self.navC.player.personalizeAds ? @"Y" : @"N")];
...
- (void)showAIOptionDialog {
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:NSLocalizedString(
                                     @"Login failed or no Internet?", nil)
                         message:
                             NSLocalizedString(
                                 @"Do you just want to play the onboard AI?",
                                 nil)
                  preferredStyle:UIAlertControllerStyleActionSheet];
    ...
    UIAlertAction *yesAction = [UIAlertAction
        actionWithTitle:NSLocalizedString(@"yes please", nil)
                  style:UIAlertActionStyleDefault
                handler:^(UIAlertAction *action) {
                    UIStoryboard *sb =
                        [UIStoryboard storyboardWithName:@"MainStoryboard"
                                                  bundle:nil];
                    MMAIViewController *vc = (MMAIViewController *)[sb
    ...
    if (alert.popoverPresentationController) {
        [alert.popoverPresentationController
            setBarButtonItem:self.navigationItem.rightBarButtonItem];
    }
