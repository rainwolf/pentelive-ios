...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [beforePicker
            ...
            forControlEvents:UIControlEventValueChanged]; // method to respond
                                                          // to changes in the
                                                          // picker value
        afterPicker = [[UIDatePicker alloc] init];
        afterPicker.datePickerMode = UIDatePickerModeDate;
        afterPicker.tag = 2;
        [afterPicker addTarget:self
                        action:@selector(datePickerValueChanged:)
              forControlEvents:UIControlEventValueChanged]; // method to respond
                                                            // to changes in the
                                                            // picker value
        ...
        UIBarButtonItem *doneButton = [[UIBarButtonItem alloc]
            initWithTitle:NSLocalizedString(@"Done", nil)
                   ...
                   target:self
                   action:@selector
                   (dismissPicker:)]; // method to dismiss the picker when the
        ...
        clearButton = [[UIBarButtonItem alloc]
            initWithTitle:NSLocalizedString(@"Clear", nil)
                   ...
                   target:self
                   action:@selector
                   (clearDate:)]; // method to dismiss the picker
        [datePickerToolbar
            setItems:[[NSArray alloc] initWithObjects:extraSpace, clearButton,
                                                      doneButton, nil]];
        ...
        gameChoices = [[NSMutableArray alloc]
...
- (void)pickerView:(UIPickerView *)pickerView
    ...
    if (pickerView.tag == 0) {
        ...
        [self setBoardColor];
        [[NSUserDefaults standardUserDefaults] setObject:gameCell.textField.text
                                                  forKey:@"DBGame"];
    } else if (pickerView.tag == 1) {
        p1RatingCell.textField.text = [ratingChoices objectAtIndex:row];
        [[NSUserDefaults standardUserDefaults]
            setObject:p1RatingCell.textField.text
               forKey:@"DBP1Rating"];
    } else if (pickerView.tag == 2) {
        p2RatingCell.textField.text = [ratingChoices objectAtIndex:row];
        [[NSUserDefaults standardUserDefaults]
            setObject:p2RatingCell.textField.text
               forKey:@"DBP2Rating"];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (indexPath.row == ++i) {
        ...
        if (cell == nil) {
            ...
            NSString *str =
                [[NSUserDefaults standardUserDefaults] objectForKey:@"DBGame"];
            if (str && [gameChoices indexOfObject:str] != NSNotFound) {
    ...
    if (indexPath.row == ++i) {
        UITableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"sortCell"];
        if (cell == nil) {
        ...
        NSString *str =
            [[NSUserDefaults standardUserDefaults] objectForKey:@"DBSort"];
        if (str) {
            cell.detailTextLabel.text = str;
        } else {
            cell.detailTextLabel.text =
                NSLocalizedString(@"win percentage", nil);
        }
    ...
    if (indexPath.row == ++i) {
        StringInputTableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"player1Cell"];
        if (cell == nil) {
            cell = [[StringInputTableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"player1Cell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    ...
    if (indexPath.row == ++i) {
        StringInputTableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"player2Cell"];
        if (cell == nil) {
            cell = [[StringInputTableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"player2Cell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    ...
    if (indexPath.row == ++i) {
        UITableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"winnerCell"];
        if (cell == nil) {
            cell = [[UITableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"winnerCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    ...
    if (indexPath.row == ++i) {
        DateTableViewCell *cell = (DateTableViewCell *)[tableView
            dequeueReusableCellWithIdentifier:@"afterCell"];
        if (cell == nil) {
            cell = [[DateTableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"afterCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];

        cell.textLabel.text = NSLocalizedString(@"After:", nil);
        //        NSString *str = [[NSUserDefaults standardUserDefaults]
        //        objectForKey: @"DBSort"]; if (str) {
        //            cell.detailTextLabel.text = str;
        //        } else {
    ...
    if (indexPath.row == ++i) {
        DateTableViewCell *cell = (DateTableViewCell *)[tableView
            dequeueReusableCellWithIdentifier:@"beforeCell"];
        if (cell == nil) {
            cell = [[DateTableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"beforeCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    ...
    if (indexPath.row == ++i) {
        InputPickerCell *cell = (InputPickerCell *)[tableView
            dequeueReusableCellWithIdentifier:@"p1RatingCell"];
        if (cell == nil) {
            cell = [[InputPickerCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"p1RatingCell"];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            NSString *str = [[NSUserDefaults standardUserDefaults]
                objectForKey:@"DBP1Rating"];
            if (str && [ratingChoices indexOfObject:str] != NSNotFound) {
                cell.textField.text = str;
    ...
    if (indexPath.row == ++i) {
        InputPickerCell *cell = (InputPickerCell *)[tableView
            dequeueReusableCellWithIdentifier:@"p2RatingCell"];
        if (cell == nil) {
            cell = [[InputPickerCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"p2RatingCell"];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
    ...
    if (indexPath.row == ++i) {
        UITableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"bothOrEitherCell"];
        if (cell == nil) {
            cell = [[UITableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"bothOrEitherCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];

        cell.textLabel.text = NSLocalizedString(@"Match players", nil);
        NSString *str = [[NSUserDefaults standardUserDefaults]
            objectForKey:@"DBBothOrEither"];
        if (str) {
            cell.detailTextLabel.text = str;
        } else {
    ...
    if (indexPath.row == ++i) {
        UITableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"excludeTimeoutCell"];
        if (cell == nil) {
            cell = [[UITableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"excludeTimeoutCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];

        cell.textLabel.text = NSLocalizedString(@"Exclude timeouts", nil);
        NSString *str = [[NSUserDefaults standardUserDefaults]
            objectForKey:@"DBExcludeTimeouts"];
        if (str) {
            cell.detailTextLabel.text = str;
        } else {
    ...
    if (indexPath.row == ++i) {
        UITableViewCell *cell =
            [tableView dequeueReusableCellWithIdentifier:@"liveOrTBCell"];
        if (cell == nil) {
            cell = [[UITableViewCell alloc]
                  initWithStyle:UITableViewCellStyleValue1
                reuseIdentifier:@"liveOrTBCell"];
        }
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];

        cell.textLabel.text = NSLocalizedString(@"Type", nil);
        NSString *str = [[NSUserDefaults standardUserDefaults]
            objectForKey:@"DBLiveOrTBCell"];
        if (str) {
            cell.detailTextLabel.text = str;
        } else {
...
- (void)tableView:(UITableView *)tableView
    ...
    if (indexPath.row == 1) {
        if ([sortCell.detailTextLabel.text
                isEqualToString:NSLocalizedString(@"popularity", nil)]) {
            [sortCell.detailTextLabel
                setText:NSLocalizedString(@"win percentage", nil)];
        } else {
            [sortCell.detailTextLabel
                setText:NSLocalizedString(@"popularity", nil)];
        }
        [[NSUserDefaults standardUserDefaults]
            setObject:sortCell.detailTextLabel.text
               forKey:@"DBSort"];
    }
    if (indexPath.row == 4) {
        if ([winnerCell.detailTextLabel.text
                isEqualToString:NSLocalizedString(@"either", nil)]) {
            [winnerCell.detailTextLabel
                setText:NSLocalizedString(@"player 1", nil)];
        } else if ([winnerCell.detailTextLabel.text
                       isEqualToString:NSLocalizedString(@"player 1", nil)]) {
            [winnerCell.detailTextLabel
                setText:NSLocalizedString(@"player 2", nil)];
        } else {
            [winnerCell.detailTextLabel
                setText:NSLocalizedString(@"either", nil)];
        }
    }
    if (indexPath.row == 9) {
        if ([eitherOrBothp1p2Cell.detailTextLabel.text
                isEqualToString:NSLocalizedString(@"either", nil)]) {
            [eitherOrBothp1p2Cell.detailTextLabel
                setText:NSLocalizedString(@"both", nil)];
        } else {
            [eitherOrBothp1p2Cell.detailTextLabel
                setText:NSLocalizedString(@"either", nil)];
        }
        [[NSUserDefaults standardUserDefaults]
            setObject:eitherOrBothp1p2Cell.detailTextLabel.text
               forKey:@"DBBothOrEither"];
    }
    if (indexPath.row == 10) {
        if ([excludeTimeoutCell.detailTextLabel.text
                isEqualToString:NSLocalizedString(@"yes", nil)]) {
            [excludeTimeoutCell.detailTextLabel
                setText:NSLocalizedString(@"no", nil)];
        } else {
            [excludeTimeoutCell.detailTextLabel
                setText:NSLocalizedString(@"yes", nil)];
        }
        [[NSUserDefaults standardUserDefaults]
            setObject:excludeTimeoutCell.detailTextLabel.text
               forKey:@"DBExcludeTimeouts"];
    }
    ...
    if (indexPath.row == 11) {
        if ([liveOrTBCell.detailTextLabel.text
                isEqualToString:NSLocalizedString(@"all", nil)]) {
            [liveOrTBCell.detailTextLabel
                setText:NSLocalizedString(@"turn-based only", nil)];
        } else if ([liveOrTBCell.detailTextLabel.text
                       isEqualToString:NSLocalizedString(@"turn-based only",
                                                         nil)]) {
            [liveOrTBCell.detailTextLabel
                setText:NSLocalizedString(@"live only", nil)];
        } else {
            [liveOrTBCell.detailTextLabel
                setText:NSLocalizedString(@"all", nil)];
        }
    }
...
- (void)setBoardColor {
    ...
    if ([gameCell.textField.text isEqualToString:@"Keryo-Pente"] ||
        [gameCell.textField.text isEqualToString:@"Speed Keryo-Pente"]) {
        ...
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.702
               ...
               [gameCell.textField.text isEqualToString:@"Speed Gomoku"]) {
        //        [gameCell.textField setText:@"Gomoku"];
        [board setBackgroundColor:[UIColor colorWithRed:0.612
                                                  green:1
                                                   blue:0.898
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.612
                                                   green:1
                                                    blue:0.898
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"D-Pente"] ||
               [gameCell.textField.text isEqualToString:@"Speed D-Pente"]) {
        //        [gameCell.textField setText:@"D-Pente"];
        [board setBackgroundColor:[UIColor colorWithRed:0.584
                                                  green:0.753
                                                   blue:0.98
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.584
                                                   green:0.753
                                                    blue:0.98
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"G-Pente"] ||
               [gameCell.textField.text isEqualToString:@"Speed G-Pente"]) {
        //        [gameCell.textField setText:@"G-Pente"];
        [board setBackgroundColor:[UIColor colorWithRed:0.616
                                                  green:0.545
                                                   blue:0.965
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.616
                                                   green:0.545
                                                    blue:0.965
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"Poof-Pente"] ||
               [gameCell.textField.text isEqualToString:@"Speed Poof-Pente"]) {
        //        [gameCell.textField setText:@"Poof-Pente"];
        [board setBackgroundColor:[UIColor colorWithRed:0.929
                                                  green:0.639
                                                   blue:0.992
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.929
                                                   green:0.639
                                                    blue:0.992
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"Connect6"] ||
               [gameCell.textField.text isEqualToString:@"Speed Connect6"]) {
        [board setBackgroundColor:[UIColor colorWithRed:0.929
                                                  green:0.639
                                                   blue:0.992
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.929
               ...
               [gameCell.textField.text isEqualToString:@"Speed Boat-Pente"]) {
        //        [gameCell.textField setText:@"Boat-Pente"];
        [board setBackgroundColor:[UIColor colorWithRed:0.145
                                                  green:0.729
                                                   blue:1
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.145
                                                   green:0.729
                                                    blue:1
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"Pente"] ||
               [gameCell.textField.text isEqualToString:@"Speed Pente"]) {
        //        [gameCell.textField setText:@"Pente"];
        ...
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.984
               ...
               [gameCell.textField.text isEqualToString:@"Speed DK-Pente"]) {
        //        [gameCell.textField setText:@"Pente"];
        [board setBackgroundColor:[UIColor colorWithRed:1
                                                  green:165.0 / 255.0
                                                   blue:0
                                                  alpha:1]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:1
                                                   green:165.0 / 255.0
                                                    blue:0
                                                   alpha:1]];
    } else if ([gameCell.textField.text isEqualToString:@"O-Pente"] ||
               [gameCell.textField.text isEqualToString:@"Speed O-Pente"]) {
        [board setBackgroundColor:[UIColor colorWithRed:0.32
                                                  green:0.75
                                                   blue:0.50
                                                  alpha:1.0]];
        [zBoard setBackgroundColor:[UIColor colorWithRed:0.32
                                                   green:0.75
                                                    blue:0.50
                                                   alpha:1.0]];
    }
...
- (id)initWithStyle:(UITableViewCellStyle)style
    ...
    if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])) {
        ...
        //        [self.ratingLabel setFont:[UIFont
        //        fontWithName:@"HelveticaNeue" size:16.f]];
...
- (void)layoutSubviews {
    ...
    CGFloat tfX = self.textLabel.frame.origin.x +
                  self.textLabel.frame.size.width + 15,
