...
//    sharedApplication] statusBarOrientation]; return (interfaceOrientation !=
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (navController.challengeCancelled) {
        ...
        if ([subject length] > 3) {
            [subjectField
                setText:([[subject substringWithRange:NSMakeRange(0, 4)]
                            isEqualToString:@"Re: "])
                            ? subject
                            : [@"Re: " stringByAppendingString:subject]];
        } else {
    ...
    [sendButton
        setFrame:CGRectMake(4, self.view.bounds.size.height - 47 - bottomOffset,
                            self.view.bounds.size.width - 8, 44)];
    ...
    float halfScreenSize =
        (self.view.frame.size.height - subjectField.frame.size.height -
         subjectField.frame.origin.y - sendButton.frame.size.height - 9 -
         bottomOffset) /
        2;
    if (messageID) {
        [self setTitle:author];
        ...
        url = [NSString stringWithFormat:@"https://www.pente.org/gameServer/"
                                         @"mymessages?command=view&mid=%@",
                                         messageID];
        if (development) {
            url = [NSString stringWithFormat:@"https://localhost/gameServer/"
                                             @"mymessages?command=view&mid=%@",
                                             messageID];
        }
        ...
        [toField
            setFrame:CGRectMake(3, 3, self.view.bounds.size.width - 6, 30)];
        self.toField.textColor = [UIColor blueColor];

        [subjectField setFrame:CGRectMake(3,
                                          toField.frame.origin.y +
                                              toField.frame.size.height + 3,
                                          self.view.bounds.size.width - 6, 30)];

        [replyMessageView
            setFrame:CGRectMake(3,
                                subjectField.frame.origin.y +
                                    subjectField.frame.size.height + 3,
                                self.view.bounds.size.width - 6, 44)];
        CGRect frame = replyMessageView.frame;
        frame.size.height = sendButton.frame.origin.y -
                            subjectField.frame.origin.y -
                            subjectField.frame.size.height - 6;
        ...
        if ([navController needHelp]) {
            ...
            [subjectField
                setText:[NSString
                            stringWithFormat:@"iOS penteLive help for %@",
                                             [[NSUserDefaults
                                                 standardUserDefaults]
                                                 objectForKey:@"username"]]];
        }

        //        [replyMessageView setFrame:CGRectMake(5, 5,
        //        self.view.bounds.size.width - 10, 44)];
    }
}
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor
        colorWithPatternImage:[UIImage imageNamed:@"preferencesBG.png"]];

    toField = [[HTAutocompleteTextField alloc]
        initWithFrame:CGRectMake(2, -32, self.view.frame.size.width - 4, 30)];
    ...
    UIView *paddingView =
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, 40, 20)];
    ...
    UILabel *tmpLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(5, 5, 40, 20)];
    ...
    subjectField = [[UITextField alloc]
        initWithFrame:CGRectMake(2, toField.frame.origin.y - 32,
    ...
    receivedMessageView = [[UITextView alloc]
        initWithFrame:CGRectMake(2, subjectField.frame.origin.y - 46,
                                 self.view.bounds.size.width - 4, 44)];
    ...
    replyMessageView = [[UITextView alloc]
        initWithFrame:CGRectMake(5, receivedMessageView.frame.origin.y - 49,
                                 self.view.bounds.size.width - 10, 44)];
    ...
    [replyMessageView setFrame:frame];
    [replyMessageView
        setAutocapitalizationType:UITextAutocapitalizationTypeSentences];
    ...
    //    sendButton = [[UIButton alloc] initWithFrame:CGRectMake(10,
    //    self.view.bounds.size.height - 44, self.view.bounds.size.width - 20,
    //    40)];
    sendButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    sendButton.backgroundColor = [UIColor whiteColor];
    sendButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold"
                                                 size:17.f];
    [sendButton setTitleColor:[UIColor blueColor]
    ...
    [sendButton setTitle:NSLocalizedString(@"send", nil)
                forState:UIControlStateNormal];
    [sendButton setFrame:CGRectMake(2, replyMessageView.frame.origin.y - 40,
...
- (void)sendButtonTap:(UIButton *)sender {
    ...
    spinner.center = CGPointMake(sendButton.frame.size.width / 4,
                                 sendButton.frame.size.height / 2);
    [spinner setHidden:NO];
    [spinner startAnimating];
    [NSThread detachNewThreadSelector:@selector(sendMessage:)
                             toTarget:self
                           withObject:replyMessageView.text];
}
...
- (void)sendMessage:(NSString *)replyMessage {
    ...
    NSString *post = [NSString
        stringWithFormat:@"command=create&to=%@&subject=%@&body=%@&mobile=",
                         author, [self URLEncodedString_ch:subject],
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
    NSURL *url =
        [NSURL URLWithString:@"https://www.pente.org/gameServer/mymessages"];
    if (development) {
        url = [NSURL URLWithString:@"https://localhost/gameServer/mymessages"];
    }
    [request setURL:url];
...
- (void)deleteMessageTap {
    ...
    [spinner setHidden:NO];
    [spinner startAnimating];
    [NSThread detachNewThreadSelector:@selector(deleteMessage)
                             toTarget:self
                           withObject:nil];
}
...
- (void)deleteMessage {
    ...
    NSString *post = [NSString
        stringWithFormat:@"command=delete&mid=%@&name2=%@&password2=%@&mobile=",
...
- (void)challenge {
    ...
    [spinner performSelectorOnMainThread:@selector(stopAnimating)
                              withObject:nil
                           waitUntilDone:NO];
    [self performSelectorOnMainThread:@selector(performChallengeSegue)
                           withObject:nil
                        waitUntilDone:NO];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"viewGameTap"]) {
        ...
        invitationVC =
            (InvitationsViewController *)segue.destinationViewController;
        [navController setUnchallengedMessageID:messageID];
...
//        (2*receivedMessageView.font.lineHeight)) {
...
//    frame.origin.y = messageID ? receivedMessageView.frame.origin.y + 10 +
//    receivedMessageView.frame.size.height : 0; [replyMessageView
//    setFrame:frame];
//
//    return YES;
//}
...
//        CGRect frame = receivedMessageView.frame;
//        if (receivedMessageView.contentSize.height <
//        (4*receivedMessageView.font.lineHeight)) {
//            frame.size.height = receivedMessageView.contentSize.height;
//        } else {
            ...
            replacementText:(NSString *)text;
{
    ...
    NSMutableString *updatedText =
        [[NSMutableString alloc] initWithString:textView.text];
    ...
    if (text.length > 2) {
        ...
        replaceRange.location -=
            2; // look back one characters (length of "hi" minus one)
    }

    @try {
        NSUInteger replaceCount =
            [updatedText replaceOccurrencesOfString:@"   "
                                         withString:@"\n"
        ...
        if (replaceCount > 0) {
            ...
            endRange.location +=
                text.length +
                replaceCount *
                    3; // length diff of "hello" and "hi" is 3 characters
        ...
        //    if (len > 2) {
        //        if ([[textView.text substringWithRange:NSMakeRange(len - 3,
        //        3)] isEqualToString:@"   "]) {
        //            text = @"\n";
        //            return YES;
        //        }
                     ...
                     if (messageID) {
                         float halfScreenSize =
                             (self.view.frame.size.height -
                              ...
                              subjectField.frame.origin.y - 3) /
                             2;
                         NSDictionary *fontAttributes = [NSDictionary
                             dictionaryWithObject:[receivedMessageView font]
                                           forKey:NSFontAttributeName];
                         CGRect textFrame = [receivedMessageView.text
                             boundingRectWithSize:CGSizeMake(
                                                      receivedMessageView
                                                          .contentSize.width,
                                                      halfScreenSize)
                         ...
                         CGFloat fontLineHeight =
                             [[receivedMessageView font] lineHeight];
                         CGRect frame = receivedMessageView.frame;
                         if ((textFrame.size.height -
                         ...
                         frame = CGRectMake(
                             ...
                             receivedMessageView.frame.origin.y +
                                 receivedMessageView.frame.size.height + 3,
                             self.view.bounds.size.width - 6, 44);
                         frame.size.height =
                             2 * halfScreenSize -
                             receivedMessageView.frame.size.height - 6;
                         frame.origin.y = receivedMessageView.frame.origin.y +
                                          3 +
                         ...
                         CGRect frame = CGRectMake(
                             ...
                             subjectField.frame.origin.y +
                                 subjectField.frame.size.height + 3,
                             self.view.bounds.size.width - 6, screenSize);
                         [replyMessageView setFrame:frame];
                     }
                 }];
}
                     ...
                     if (messageID) {
                         ...
                         [replyMessageView
                             setFrame:CGRectMake(
                                          3,
                                          receivedMessageView.frame.origin.y +
                                              receivedMessageView.frame.size
                                                  .height +
                                              3,
                                          self.view.bounds.size.width - 6, 44)];
...
- (NSString *)stripURL:(NSString *)inStr {
    NSString *tmpStr = [inStr stringByReplacingOccurrencesOfString:@"</a>"
                                                        withString:@""],
             *tmpTmpStr;
    ...
    while (urlRange.location != NSNotFound) {
        tmpRange = [tmpStr rangeOfString:@"\">"
                                 options:0
                                   range:NSMakeRange(urlRange.location,
                                                     len - urlRange.location)];
        tmpTmpStr = [tmpStr
        ...
        if (stringToLookFor && [stringToCompare hasPrefix:stringToLookFor]) {
            return [stringFromReference
                stringByReplacingCharactersInRange:
                    [stringToCompare rangeOfString:stringToLookFor]
                                        withString:@""];
        }
    }
...
- (BOOL)textView:(UITextView *)textView
    ...
    if ([urlString rangeOfString:@"mobile&g="].location != NSNotFound) {
        ...
        //        [game setNameColor: UIColorFromRGB([[splitLine
        //        objectAtIndex:7] intValue])];
    ...
    } else if ([urlString rangeOfString:@"new.jsp?game="].location !=
               NSNotFound) {
        [self performSegueWithIdentifier:@"challengeSegue" sender:self];

        NSString *gameString = nil;
        if ([urlString rangeOfString:@"new.jsp?game=51&"].location !=
            NSNotFound) {
            gameString = @"Pente";
        } else if ([urlString rangeOfString:@"new.jsp?game=53&"].location !=
                   NSNotFound) {
            gameString = @"Keryo-Pente";
        } else if ([urlString rangeOfString:@"new.jsp?game=55&"].location !=
                   NSNotFound) {
            gameString = @"Gomoku";
        } else if ([urlString rangeOfString:@"new.jsp?game=57&"].location !=
                   NSNotFound) {
            gameString = @"D-Pente";
        } else if ([urlString rangeOfString:@"new.jsp?game=59&"].location !=
                   NSNotFound) {
            gameString = @"G-Pente";
        } else if ([urlString rangeOfString:@"new.jsp?game=61&"].location !=
                   NSNotFound) {
            gameString = @"Poof-Pente";
        } else if ([urlString rangeOfString:@"new.jsp?game=63&"].location !=
                   NSNotFound) {
            gameString = @"Connect6";
        } else if ([urlString rangeOfString:@"new.jsp?game=65&"].location !=
                   NSNotFound) {
