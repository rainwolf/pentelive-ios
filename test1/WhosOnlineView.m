...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    Player *playr = [[rooms objectAtIndex:indexPath.section].players
        objectAtIndex:indexPath.row];
    [cell setAccessoryType:UITableViewCellAccessoryDisclosureIndicator];
    NSMutableString *txtStr =
        [[NSMutableString alloc] initWithString:[playr name]];
    ...
    default:
        if (crown > 3) {
            textAttachment.image =
                [UIImage imageNamed:[NSString stringWithFormat:@"kothcrown%i",
                                                               crown - 3]];
        }
        break;
    }
    ...
    NSMutableAttributedString *tmpStr =
        [[NSMutableAttributedString alloc] initWithString:ratingStr];
    ...
    if ([playr color] != 0 || [[[rooms objectAtIndex:indexPath.section] name]
                                  isEqualToString:@"Mobile"]) {
        [tmpStr addAttribute:NSForegroundColorAttributeName
                       value:UIColorFromRGB([playr color])
                       range:NSMakeRange(0, [[playr name] length])];
    }
    [tmpStr addAttribute:NSFontAttributeName
                   value:[UIFont fontWithName:@"HelveticaNeue" size:16.f]
    ...
    if (crown > 0) {
        [tmpStr appendAttributedString:[[NSMutableAttributedString alloc]
                                           initWithString:@" "]];
    ...
    if ([[[rooms objectAtIndex:indexPath.section] name]
            isEqualToString:@"Mobile"]) {
        ...
        tmpStr = [[NSMutableAttributedString alloc]
            initWithAttributedString:((PlayerCell *)cell)
                                         .ratingLabel.attributedText];
        [tmpStr addAttribute:NSForegroundColorAttributeName
                       value:[UIColor blackColor]
        ...
        cell.detailTextLabel.text = [NSString
            stringWithFormat:NSLocalizedString(@"Total games: %@", nil),
                             [playr numberOfGames]];
    }
    ...
    //    NSMutableString *ratingStr = [NSMutableString
    //    stringWithString:@"\u25A0
    //    "]; if ([[players objectAtIndex: indexPath.row].rating length] == 3) {
    //        [ratingStr appendString:@"  "];
    //    }
    //    [ratingStr appendString: [players objectAtIndex:
    //    indexPath.row].rating]; NSMutableAttributedString *tmpStr =
    //    [[NSMutableAttributedString alloc] initWithString: ratingStr]; [self
    //    addColorOfRating: [players objectAtIndex: indexPath.row].rating
    //    toString: tmpStr]; cell.ratingLabel.attributedText = tmpStr; [cell
...
- (void)tableView:(UITableView *)tableView
    ...
    //    NSLog(@"didselect");
    if ([[self tableView:tableView titleForHeaderInSection:indexPath.section]
            isEqualToString:@"Mobile"]) {
        //        NSLog(@"mobile");
        [vc toInvitationsWithPlayer:[[rooms objectAtIndex:indexPath.section]
                                            .players
                                        objectAtIndex:indexPath.row]
                                        .name];
    } else {
        ...
        //        NSLog(@"else %@", [[rooms
        //        objectAtIndex:indexPath.section].players
        ...
        NSString *address = [NSString
            stringWithFormat:@"https://www.pente.org/gameServer/"
                             ...
                             [[rooms objectAtIndex:indexPath.section].players
                                 objectAtIndex:indexPath.row]
                                 .name,
                             username, password];
    ...
    //    objectForKey:@"username"]; NSString *game = nil; if ([[[ratingStats
    //    objectAtIndex: indexPath.row] game] isEqualToString:@"Pente"])
    //        game = @"51";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
    //    isEqualToString:@"Gomoku"])
    //        game = @"55";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
    //    isEqualToString:@"D-Pente"])
    //        game = @"57";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
    //    isEqualToString:@"G-Pente"])
    //        game = @"59";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
    //    isEqualToString:@"Boat-Pente"])
    //        game = @"65";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
    //    isEqualToString:@"Poof-Pente"])
    //        game = @"61";
    //    if ([[[ratingStats objectAtIndex: indexPath.row] game]
...
- (void)layoutSubviews {
    ...
    if (self.imageView.image) {
        [self.imageView
            setFrame:CGRectMake(0, 0, self.contentView.frame.size.height,
    ...
    [self.ratingLabel
        setFrame:CGRectMake(
                     ...
                     2, (screenWidth - imageWidth - accessoryWidth - 20) / 2,
                     22)];
