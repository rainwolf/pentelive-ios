...
- (NSInteger)tableView:(UITableView *)tableView
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"showOnlyTB"]) {
        return ((PenteNavigationViewController *)vc.navigationController)
            .player.tbRatings;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    RatingStatCell *cell = (RatingStatCell *)[tableView
        dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell =
            [[RatingStatCell alloc] initWithStyle:UITableViewCellStyleSubtitle
                                  reuseIdentifier:cellIdentifier];
    }
    cell.textLabel.text = [ratingStats objectAtIndex:indexPath.row].game;
    cell.detailTextLabel.text = [NSString
        stringWithFormat:NSLocalizedString(@"Last: %@, total: %@", nil),
                         [ratingStats objectAtIndex:indexPath.row].lastPlayed,
                         [ratingStats objectAtIndex:indexPath.row].totalGames];
    ...
    [self addColorOfRating:[ratingStats objectAtIndex:indexPath.row].rating
                  toString:tmpStr];
...
- (void)tableView:(UITableView *)tableView
    ...
    NSString *urlString = [NSString
        stringWithFormat:@"https://www.pente.org/gameServer/"
                         @"viewLiveGames?p=%@&g=%i&name2=%@&password2=%@",
                         username, gameInt, username, password];
    if (development) {
        urlString = [NSString
...
- (void)layoutSubviews {
    ...
    [self.textLabel
        setFrame:CGRectMake(10, 2, (screenWidth - accessoryWidth + 60) / 2,
                            22)];
    [self.ratingLabel
        setFrame:CGRectMake(10 + (screenWidth - accessoryWidth - 20) / 2, 2,
                            (screenWidth + accessoryWidth - 20) / 2, 22)];
    ...
    //    [self.textLabel setFrame:CGRectMake(imageWidth + 10, 2, (screenWidth -
    //    imageWidth - accessoryWidth + 60)/2, 22)]; [self.ratingLabel
    //    setFrame:CGRectMake(imageWidth + 10 + (screenWidth - imageWidth -
    //    accessoryWidth - 20)/2, 2, (screenWidth - accessoryWidth - 60)/2,
    //    22)]; [self.detailTextLabel setFrame:CGRectMake(imageWidth + 10, 24,
    //    screenWidth - imageWidth - accessoryWidth - 20, 18)]; [self.imageView
    //    setFrame:CGRectMake(0, 0, imageWidth, imageWidth)];
