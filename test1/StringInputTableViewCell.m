...
- (void)initalizeInputView {
    ...
    self.textField = [[HTAutocompleteTextField alloc]
        initWithFrame:CGRectMake(self.frame.size.width * 4 / 9,
                                 (self.frame.size.height -
                                  [UIFont systemFontOfSize:17.0f].lineHeight) /
                                     2,
                                 self.frame.size.width -
                                     self.frame.size.width * 4 / 9 - 10,
                                 [UIFont systemFontOfSize:17.0f].lineHeight)];
    ...
    //    arrayWithObjects:@"iostesterrrrrrrrrrrrrrrrrrr",@"iostest", @"selwa",
    //    nil]; self.invitedHistory = (NSMutableArray *) [testArray
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    //    self.textField.frame = CGRectMake(self.frame.size.width/2,
    //    (self.frame.size.height - [UIFont systemFontOfSize:17.0f].lineHeight)
    //    / 2 , self.frame.size.width/2 -20, [UIFont
    //    systemFontOfSize:17.0f].lineHeight);
    self.textField.textAlignment = NSTextAlignmentLeft;
    id tableView = self;
    while (![tableView isKindOfClass:[UITableView class]] &&
           [tableView respondsToSelector:@selector(superview)]) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    //    self.textField.frame = CGRectMake(self.frame.size.width/3,
    //    (self.frame.size.height - [UIFont systemFontOfSize:17.0f].lineHeight)
    //    / 2 , 2*self.frame.size.width/3-20, [UIFont
    //    systemFontOfSize:17.0f].lineHeight);
    self.textField.textAlignment = NSTextAlignmentRight;
    if (delegate && [delegate respondsToSelector:@selector
                              (tableViewCell:didEndEditingWithString:)]) {
...
- (void)layoutSubviews {
    ...
    self.textField.frame = CGRectMake(
        self.frame.size.width * 4 / 9,
        (self.frame.size.height - [UIFont systemFontOfSize:17.0f].lineHeight) /
            2,
        self.frame.size.width - self.frame.size.width * 4 / 9 - 10,
        [UIFont systemFontOfSize:17.0f].lineHeight);
