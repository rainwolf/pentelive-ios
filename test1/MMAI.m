...
int rotx[] = {1, 1, 1, 1, -1, -1, -1, -1};
int roty[] = {1, 1, -1, -1, -1, -1, 1, 1};
int rotf[] = {0, 1, 0, 1, 0, 1, 0, 1};
// int mxnd[] = {0, 20, 18, 16, 14, 12, 12, 12, 12, 12, 8,8,8,8,8,8, 8,8,8};
int mxvt[] = {0,  1,  3,  4,  6,  7,  8,  9,  10, 12,
              13, 14, 15, 18, 18, 18, 18, 18, 18};
int mxvf[] = {0,  1,  4,  5,  7,  8,  10, 11, 13, 15,
...
const int op2[] = {25, 36, 77, 82, 93, 95, 97, 99};
const int om3[] = {183, 184, 202, 221, 240, 260, 239, 238, 237, 256, 236,
                   235, 234, 252, 215, 196, 177, 176, 158, 139, 120, 100,
...
short int *pHashS;
int exfl[20], exel[20];
int maxscr, alpha, beta, extnt, extent, breadth;
unsigned int *pHashY, HValX[20], HValY[20], TableX[362], TableY[362];
int bd[19][19], cc[20][7], p1d[24], p2d[24], p3d[24], cap2, cap3;

CGPoint p1xy[24], pxy[24], p2xy[24], p3xy[24];
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self reset];
        NSString *path = [[NSBundle mainBundle] pathForResource:@"pente"
                                                         ofType:@"tbl"];
        NSData *fileData = [NSData dataWithContentsOfFile:path];
        unsigned char *fileDataArray = (unsigned char *)[fileData bytes];
        int fileCntr = 0;
        for (int i = 0; i < 943; i++) {
            for (int j = 0; j < 4; j++) {
                if (fileCntr < [fileData length] - 1) {
                    table[i * 4 + j] =
                        (((short int)fileDataArray[fileCntr + 1]) << 8) |
                        fileDataArray[fileCntr];
                    fileCntr += 2;
                }
            }
        }
        ...
        path = [[NSBundle mainBundle] pathForResource:@"opngbk" ofType:@"pen"];
        fileData = [NSData dataWithContentsOfFile:path];
        fileDataArray = (unsigned char *)[fileData bytes];
        fileCntr = 0;

        obsize =
            (((int)fileDataArray[fileCntr + 1]) << 8) | fileDataArray[fileCntr];
        ...
        if (obsize < openingBookSize) {
            ...
            do {
                i = cob;
                if (fileCntr < [fileData length] - 1) {
                    sint = (((short int)fileDataArray[fileCntr + 1]) << 8) |
                           fileDataArray[fileCntr];
                    ef = sint;
                } else {
                    sint = -1;
                ...
                if (ef != -1) {
                    ...
                    for (int j = 0; j < nom[i]; j++) {
                        if (fileCntr < [fileData length] - 1) {
                            sint = (((short int)fileDataArray[fileCntr + 1])
                                    << 8) |
                                   fileDataArray[fileCntr];
                            ef = sint;
                        } else {
...
- (void)addMove:(int)move {
    [moves addObject:[NSNumber numberWithInt:move]];
...
- (int)getMove {
    NSArray *oldMoves = moves;
    [self reset];
    for (NSNumber *move in oldMoves) {
        [self addMove:[move intValue]];
    }
    moveNum = [oldMoves count];
    tn = moveNum + 1;
    //    int move = [self Move];
    int move = [self cmove];
    if (move == -1) {
...
- (void)dmov {
    ...
    for (x = sx[tn] - extnt; x < sx[tn] + 1 + extnt;
         x++) // set spaces around piece to -1
        for (y = sy[tn] - extnt; y < sy[tn] + 1 + extnt;
             y++) // for consideration by ai
    ...
    if (tn > 1 && obfl != 0) {
        ...
        for (i = 0; i < 8; i++) {
            for (obi = 0; obi < obsize; obi++) {
                ...
                if (p[3 - cp] != 0 && mfl != 0 &&
                    ((cp == 2 && oscr[obi] > 5) || (cp == 1 && oscr[obi] < 5)))
                    // if (p[3-cp] && mfl && (cp==2 && oscr[obi]>5 || cp==1 &&
                    // oscr[obi]<5))
...
- (int)Move { // AI MAIN routine
    ...
    mxnd[1] = 20;
    mxnd[2] = 18; // number of nodes to expand at each level
    mxnd[3] = 16;
    mxnd[4] = 14;
    ...
    do {     // A
            ...
            if (vct) { // VCT
                ...
                *pexel = exel[lvl - 1];
                ...
                if (*pmxmv < 2)
                    ctfl = 0;
                if (fr == cp && ctfl) {
                    if (sco[fr] < 520 && *pexfl > 1 && lvl >= mxst &&
                        lvl > mxvt[plv])
                        *pexfl = *pexfl - 2;
                    if (sco[fr] < 110 && *pexfl > 1 && lvl >= mxst)
                        *pexfl = *pexfl - 2;
                    if (sco[fr] < 110 && (*pexfl) % 2 && sc < 1800)
                        (*pexfl)--;
                }
                if (fr != cp && ctfl) {
                    if (sco[fr] < 520 && *pexel > 1 && lvl >= mxst &&
                        lvl > mxvt[plv])
                        *pexel = *pexel - 2;
                    if (sco[fr] < 110 && *pexel > 1 && lvl >= mxst)
                        *pexel = *pexel - 2;
                    if (sco[fr] < 110 && (*pexel) % 2 && sc < 1800)
                        (*pexel)--;
                }
                if (*pexfl < 2 && *pexel < 2 && lvl >= mxvt[plv])
                    mxlv = lvl;
                if (!(*pexfl) && !(*pexel) && lvl >= plv)
                    mxlv = lvl;
                if (lvl <= plv)
                    mxlv = plv;
                if ((*pexfl > 1 || *pexel > 1) && lvl == mxlv &&
                    lvl < mxvf[plv])
                    mxlv++;
                if (((*pexfl) % 2 || (*pexel) % 2) && lvl == mxlv &&
                    lvl < mxvt[plv])
                    mxlv++;
            } // end vct
            else
...
#if HASH == 1
                htempx =
                    (HValX[lvl] ^ (TableX[361] * (cc[lvl][1] + cc[lvl][2]))) %
                    1000000;
        ...
        if (lvl > 0) {
            if (scr[lvl][fr] > scr[lvl - 1][fr] &&
                ...
                if (lvl < mxlv) {
                    htempx = (HValX[lvl] ^
                              (TableX[361] * (cc[lvl][1] + cc[lvl][2]))) %
                             1000000;
                    htempy =
                        HValY[lvl] ^ (TableY[361] * (cc[lvl][1] + cc[lvl][2]));
            ...
            if (np == 2 && lvl == 1 && scr[0][cp] < 12000 &&
                12000 - scr[0][cp] < ciel[0][cp])
...
- (int)Score:(CGPoint)pt {
    ...
    do {                            // c0
            ...
            for (sign = -1; sign < 2; sign += 2) { // look for captures
                ...
                if (la[1] > 0 && la[1] != fr)
                    if (la[2] > 0 && la[2] != fr) { // b0
                        if (la[3] == fr)
                            c4 += 2; // pair now open for capture
                        if (!la[3])
                            c5 += 2; // pair no longer open for capture
        ...
        if (dv < 4) {
            ...
            for (i = 1; i <= np; i++)
                c2[i] = 0;
            sp[0] = sp[1] = 0;
            if (g[0] == 8 || g[0] == 3 || g[0] == 5)
                sp[0] = 1;
            if (g[1] == 8 || g[1] == 3 || g[1] == 5)
                sp[1] = 1;
            for (i = 0; i < 2; i++) {
                ...
                if (Kgame && g[i] == 7 && sp[1 - i])
                    sco[fr] -= 12;
                if (!Kgame && g[i] == 7)
                    sco[fr] += 12;
                if (g[i] == 2) { // threaten a pair
                    sco[fr] += 50;
                    if (cc[lvl][fr] + cap1 > Kgame * 5 + 7)
                        sco[fr] += 1024;
                }
                if (Kgame && g[i] == 4) {
                    sco[fr] += 75;
                    if (cc[lvl][fr] + cap1 > Kgame * 5 + 6)
                        sco[fr] += 1024;
                }
            } // next i
            if (Kgame && g[0] == 6 && g[1] == 6)
                sco[fr] -= 12;              // pair
            for (iw = 0; iw < 5; iw += 4) { // O = played
                for (i = 1; i <= np; i++) { // Z = potential capturer
                    if (i != fr && lb[iw + 1] > 0 && lb[iw + 2] > 0) // OXYZ
                        if (lb[iw + 1] != i && lb[iw + 2] != i) {    // protect
                            if (lb[iw + 3] == i && (!Kgame || lb[5 - iw]))
                                c3[i] += 2;
                            if (Kgame && lb[iw + 3] != i && lb[iw + 3] > 0 &&
                                lb[iw + 4] == i)
                                c3[i] += 3;
                        }
                } // next i
                ...
                if (i > 0 && i != fr)
                    if (lb[iw + 1] > 0 && lb[iw + 1] != i) { // _XOZ
                        if (!lb[iw + 2])
                            c2[i] += 2; // make suscept. pair
                        if (Kgame && lb[iw + 2] > 0 && lb[iw + 2] != i &&
                            !lb[iw + 3])
                            c2[i] += 3;
                    }
                i = lb[6 - iw];
                if (i > 0 && i != fr)
                    if (lb[5 - iw] > 0 && lb[5 - iw] != i) { // _OXZ
                        if (!lb[iw + 1])
                            c2[i] += 2;
                        if (Kgame && lb[iw + 1] > 0 && lb[iw + 1] != i &&
                            !lb[iw + 2])
                            c2[i] += 3;
                    }
                i = lb[7 - iw];
                if (i > 0 && i != fr && Kgame) // _OXYZ
                    if (lb[6 - iw] > 0 && lb[6 - iw] != i && lb[5 - iw] > 0 &&
                        lb[5 - iw] != i)
                        if (!lb[iw + 1])
                            c2[i] += 3;
            } // next iw
    ...
    if (cc[lvl][fr] + c4 > Kgame * 5 + 9)
        s0 = s0 + 1024;
    if (cc[lvl][fr] + c5 > Kgame * 5 + 9)
        s0 = s0 - 1024;
    sco[fr] -= s0; // is subtracted in eval
    if (cc[lvl][fr] + cap1 > Kgame * 5 + 9)
        sco[fr] = 12000;
