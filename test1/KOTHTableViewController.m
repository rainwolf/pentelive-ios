...
- (id)initWithStyle:(UITableViewCellStyle)style
    ...
    if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])) {
        ...
        [self.ratingLabel setFont:[UIFont fontWithName:@"HelveticaNeue"
                                                  size:16.f]];
        [self.textLabel setFont:[UIFont fontWithName:@"HelveticaNeue"
                                                size:16.f]];
...
- (void)layoutSubviews {
    ...
    [self.textLabel
        setFrame:CGRectMake(
                     imageWidth + 10, 3,
                     (screenWidth - imageWidth - accessoryWidth + 60) / 2, 22)];
    [self.ratingLabel
        setFrame:CGRectMake(
                     imageWidth + 10 +
                         (screenWidth - imageWidth - accessoryWidth - 20) / 2,
                     2, (screenWidth - imageWidth - accessoryWidth - 60) / 2,
                     22)];
    ...
    if (dark) {
        if (@available(iOS 13.0, *)) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle ==
                ...
                [txtStr addAttribute:NSForegroundColorAttributeName
                               value:[UIColor blackColor]
                               range:NSMakeRange(2, [txtStr length] - 2)];
            }
        }
    }
    return txtStr;
    ...
    if (hillSummary.member && hillSummary.gameId > 50) {
        UIBarButtonItem *barButton = [[UIBarButtonItem alloc]
            initWithBarButtonSystemItem:UIBarButtonSystemItemAction
                                 target:self
                                 action:@selector(loadWebsiteHill)];
        UIBarButtonItem *inviteButton = [[UIBarButtonItem alloc]
            initWithImage:[UIImage imageNamed:@"person.png"]
                    style:UIBarButtonItemStylePlain
                   target:self
                   action:@selector(showOpenChallengeView)];
        //    statsButton = [[UIBarButtonItem alloc]
        //    initWithBarButtonSystemItem: UIBarButtonSystemItem target:self
        //    action: @selector(showStats)]; friendsButton = [[UIBarButtonItem
        //    alloc] initWithImage: [UIImage imageNamed:@"friends.png"] style:
        //    UIBarButtonItemStylePlain target:self
        //    action:@selector(toFriends)];
        [self.navigationItem setRightBarButtonItem:nil];
        [self.navigationItem
            setRightBarButtonItems:[NSArray arrayWithObjects:barButton,
                                                             inviteButton,
                                                             nil]];
    } else {
        UIBarButtonItem *barButton = [[UIBarButtonItem alloc]
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 0) {
        cell = (UITableViewCell *)[tableView
            dequeueReusableCellWithIdentifier:@"joinLeaveCell"];
        if (!cell) {
            //            cell = [[UITableViewCell alloc]
            //            initWithStyle:UITableViewCellStyleDefault
            //            reuseIdentifier:@"joinLeaveCell"];
        ...
        Player *playr = [[[hill steps] objectAtIndex:indexPath.section - 1]
            objectAtIndex:indexPath.row];
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"username"]
                isEqualToString:[playr name]]) {
            cell = (PlayerTableViewCell *)[tableView
                dequeueReusableCellWithIdentifier:@"myKothCell"];
            if (!cell) {
                cell = [[PlayerTableViewCell alloc]
                      initWithStyle:UITableViewCellStyleSubtitle
                    reuseIdentifier:@"myKothCell"];
            }
            ...
            cell = (PlayerTableViewCell *)[tableView
                dequeueReusableCellWithIdentifier:@"kothCell"];
            if (!cell) {
                cell = [[PlayerTableViewCell alloc]
                      initWithStyle:UITableViewCellStyleSubtitle
                    reuseIdentifier:@"kothCell"];
            }
            NSString *opponent = [playr name];
            UIImage *imgV = [player.avatars objectForKey:opponent];
            cell.imageView.image = imgV;
    ...
    if (indexPath.section == 0) {
        if (hillSummary.gameId < 50) {
            cell.textLabel.text = NSLocalizedString(
                @"join by playing in the live KotH room", nil);
        } else {
            if ([hillSummary member]) {
                cell.textLabel.text =
                    NSLocalizedString(@"Tap to leave this hill", nil);
            } else {
                cell.textLabel.text =
                    NSLocalizedString(@"Tap to join this hill", nil);
            }
        }
        if (![player subscriber] && ![hillSummary member]) {
            cell.detailTextLabel.text = NSLocalizedString(
                @"Warning: only subscribers can join multiple hills", nil);
        }
        ...
        ((PlayerTableViewCell *)cell).ratingLabel.attributedText =
            [playr ratingString:[playr canBeChallenged]];
        cell.textLabel.attributedText = [player
                 markIfOnline:[playr name]
            andAttributedName:[playr attributedName:[playr canBeChallenged]]];
        NSMutableAttributedString *detailStr = [[NSMutableAttributedString
            alloc]
        ...
        if ([playr canBeChallenged]) {
            cell.backgroundColor = [UIColor colorWithRed:222.0 / 256
                                                   green:236.0 / 256
                                                    blue:222.0 / 256
                                                   alpha:1];
            [detailStr addAttribute:NSForegroundColorAttributeName
                              value:[UIColor blackColor]
...
- (void)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 0) {
        ...
        if (hillSummary.gameId > 50 && [playr canBeChallenged]) {
            ...
            challengeView = [[KOTHChallengeView alloc]
            ...
            NSString *url = [NSString
                stringWithFormat:@"https://www.pente.org/gameServer/"
                                 @"profile?viewName=%@&name2=%@&password2=%@",
                                 playr.name, username, password];
            if (development) {
                url =
                    [NSString stringWithFormat:
                                  @"https://localhost/gameServer/"
                                  @"profile?viewName=%@&name2=%@&password2=%@",
                                  playr.name, username, password];
            }
...
- (NSString *)tableView:(UITableView *)tableView
    ...
    if (section == 0) {
        return [NSString
            stringWithFormat:NSLocalizedString(@"King of the Hill - %@", nil),
                             [hillSummary game]];
    } else if (section == 1) {
        return NSLocalizedString(@"top of the hill", nil);
    } else {
        return [NSString stringWithFormat:NSLocalizedString(@"step %lu", nil),
                                          [hill.steps count] - section + 1];
    }
}
...
- (void)showOpenChallengeView {
    if (![hillSummary canSendOpen] && !player.subscriber) {
        ...
        UIAlertController *alertDialog = [UIAlertController
            alertControllerWithTitle:NSLocalizedString(@"KotH limit", nil)
                             message:NSLocalizedString(
                                         @"Free players can play, at most, 2 "
                                         @"KotH sets "
                                         @"at a time, this includes sent and "
                                         @"received "
                                         @"invitations, you can post "
                                         @"invitations again "
                                         @"after you finish a set.\n "
                                         @"Subscribers don't "
                                         @"have this limit.",
                                         nil)
        ...
        if (alertDialog.popoverPresentationController != nil) {
            alertDialog.popoverPresentationController.barButtonItem =
        ...
        challengeView = [[KOTHChallengeView alloc]
            initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width * 4 / 5,
                                     147)];
        [challengeView setScrollEnabled:NO];
        [challengeView setGameId:[hillSummary gameId]];
...
- (void)addColorOfRating:(NSString *)rating
                toString:(NSMutableAttributedString *)str {
    int ratingInt = [rating intValue];
    UIColor *ratingColor;
    if (ratingInt >= 1900) {
        ratingColor = [UIColor redColor];
...
- (void)joinLeave {
    ...
    NSString *postString =
        [NSString stringWithFormat:@"%@&game=%i",
                                   [hillSummary member] ? @"leave=" : @"join=",
                                   hillSummary.gameId];
...
- (void)loadKoth {
    ...
    // connect to the game server
    url = [NSString
        stringWithFormat:@"https://www.pente.org/gameServer/mobile/"
                         @"koth.jsp?name=%@&game=%i&name2=%@&password2=%@",
                         username, gameId, username, password];
    if (development) {
        username = @"iostest";
        url = [NSString
            stringWithFormat:@"https://localhost/gameServer/mobile/"
                             @"koth.jsp?name=%@&game=%i&name2=%@&password2=%@",
                             username, gameId, username, password];
    }
    [request setURL:[NSURL URLWithString:url]];
    ...
    [hillSummary setMember:[dashboardString rangeOfString:username].location !=
                           NSNotFound];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [CATransaction setCompletionBlock:^{
            ...
            //            withObject: self.tableView afterDelay:0.01];
        }];

        //        [self.tableView setUserInteractionEnabled:NO];

        //        if ([dashboardString rangeOfString:@"Invitations
        //        received"].location == NSNotFound) {
        //            self.tableView.layer.borderWidth = 0.0;
        ...
        //        while ((dashIDX < [splitDash count]) && ([[splitDash
        //        objectAtIndex:dashIDX] isEqualToString: @""])) {
        //            dashIDX++;
        //        }
        //        if ((dashIDX < [splitDash count])) {
        //            while ((![[splitDash objectAtIndex:dashIDX]
        //            isEqualToString:
        //            @""]) && (dashIDX < [splitDash count])) {
        while (dashIDX < [splitDash count]) {
            ...
            if ([splitLine count] > 0) {
                for (NSString *playerStr in splitLine) {
                    NSArray<NSString *> *playerDetails =
                        [playerStr componentsSeparatedByString:@","];
                    ...
                    [kothPlayer
                        setCanBeChallenged:[[playerDetails objectAtIndex:2]
                                               isEqualToString:@"yes"]];
                    [kothPlayer
                        setColor:[[playerDetails objectAtIndex:3] intValue]];
                    [kothPlayer
                        setCrown:[[playerDetails objectAtIndex:4] intValue]];
        ...
        //        }
        while ([[hill steps] count] > 0 &&
               [[[hill steps] firstObject] count] == 0) {
            [[hill steps] removeObjectAtIndex:0];
        }

        //        indexSet = [[NSMutableArray alloc] init];
        //        for(int i = 0; i < [[player hills] count]; ++i)
        //            [indexSet addObject:[NSIndexPath indexPathForRow: i
        //            inSection: KOTHSECTION]];
        //        [player setHills:[[NSMutableArray alloc] init]];
        //        [self.tableView deleteRowsAtIndexPaths:indexSet
        //        withRowAnimation:UITableViewRowAnimationFade]; [player
        //        setHills:sectionItems]; indexSet = [[NSMutableArray alloc]
        //        init]; for(int i = 0; i < [[player hills] count]; ++i)
        //            [indexSet addObject:[NSIndexPath indexPathForRow: i
        //            inSection: KOTHSECTION]];
...
- (UIView *)tableView:(UITableView *)tableView
    ...
    if (sectionHeaderView == nil) {
        ...
        headerLabel = [[UILabel alloc]
            initWithFrame:CGRectMake(12, 2, tableView.bounds.size.width - 60,
                                     18)];
        ...
        collapsedLabel = [[UILabel alloc]
            initWithFrame:CGRectMake(tableView.bounds.size.width - 70, 2, 70,
                                     18)];
        collapsedLabel.center =
            CGPointMake(tableView.bounds.size.width - 40, 11);
    ...
    if (section > 0) {
        ...
        for (Player *playr in [[hill steps] objectAtIndex:section - 1]) {
            if ([[[NSUserDefaults standardUserDefaults]
                    objectForKey:@"username"] isEqualToString:[playr name]]) {
                [sectionHeaderView
                    setBackgroundColor:[UIColor colorWithRed:1.0
                                                       green:(129.0 / 255)
                                                        blue:0.0
                                                       alpha:1.0]];
                break;
        ...
        collapsedLabel.text = [NSString
            stringWithFormat:@"(%lu)", (unsigned long)[[hill.steps
                                           objectAtIndex:section - 1] count]];
    } else {
        collapsedLabel.text =
            [NSString stringWithFormat:@"(%@)", [hillSummary numPlayers]];
        [sectionHeaderView setBackgroundColor:[UIColor colorWithRed:(8.0 / 255)
                                                              green:(52.0 / 255)
                                                               blue:(29.0 / 255)
                                                              alpha:1.0]];
...
- (void)loadWebsiteHill {
    NSString *urlString = [NSString
        stringWithFormat:@"https://www.pente.org/gameServer/stairs.jsp?game=%i",
                         hillSummary.gameId];
    if (development) {
        urlString = [NSString
            stringWithFormat:@"https://localhost/gameServer/stairs.jsp?game=%i",
                             hillSummary.gameId];
    }
