...
- (void)viewDidLoad {
    ...
    //    games = [[NSArray alloc]
    //    initWithObjects:@"Pente",@"Keryo-Pente",@"Gomoku",@"D-Pente",@"G-Pente",@"Poof-Pente",@"Connect6",@"Boat-Pente",
    //    nil]; games = [[NSMutableArray alloc]
    //    initWithObjects:@"Pente",@"Gomoku", nil];
    games = [[NSMutableArray alloc] initWithObjects:@"Pente", nil];
    colors = [[NSMutableArray alloc]
        initWithObjects:NSLocalizedString(@"white", nil),
    ...
    ratedCell.textLabel.text =
        NSLocalizedString(@"Play as white and black", nil);
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    //        [timeCell.detailTextLabel setText:[defaults
    //        objectForKey:@"lastInvitedTimeLimit"]];
    //    }
    ...
    //        [playAsCell.detailTextLabel setText:[defaults
    //        objectForKey:@"lastInvitedColor"]];
    //    }
...
- (void)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == row) {
            ...
            //            [tableView scrollRectToVisible:CGRectMake(0, 0, 1, 1)
            //            animated:NO]; [tableView setScrollEnabled:YES];
        }
        ++row;
        if (indexPath.row == row) {
            [gameCell doResign];
            ...
            //            [tableView scrollToRowAtIndexPath:indexPath
            //            atScrollPosition:UITableViewScrollPositionTop
            //            animated:YES]; [tableView setScrollEnabled:NO];
        }
        ++row;
        if (indexPath.row == row) {
            [opponentCell resign];
...
- (void)submitInvitation {
    ...
    NSString *post =
        [NSString stringWithFormat:
                      @"invitee=computer&game=%@&difficulty=%@&daysPerMove=30&"
                      @"rated=%@&playAs=%@&privateGame=N&mobile=",
                      gameString, opponentCell.detailTextLabel.text,
                      (ratedSwitch.on) ? @"Y" : @"N",
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [defaults setObject:opponentCell.detailTextLabel.text
